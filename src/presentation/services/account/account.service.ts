...
// Dependencies Injection
  ...
  async loadAccountById(id: string, payload: TokenPayload) {
  ...
  async loadAccountByUsername(params: LoadAccountByEmailInput, payload: TokenPayload) {
    const response = await this.queryBus.execute(new LoadAccountByEmailQuery(params, payload))
    return baseResponse(200, 'Conta recuperada com sucesso!', response)
  }

  // Load me query
  async loadMe(payload: TokenPayload) {
    const response = await this.queryBus.execute(new LoadMeQuery(payload))
    return baseResponse(200, 'Conta recuperada com sucesso!', response)
  }

  // Add account command
  async addAccount(params: AddAccountInput) {
  ...
  async updateAccount(id: string, params: UpdateAccountInput, payload: TokenPayload) {
  ...
  async removeAccount(id: string, payload: TokenPayload) {
  ...
  async verifyAccount(params: VerifyAccountInput, payload: TokenPayload) {
  ...
  async resendVerification(payload: TokenPayload) {
