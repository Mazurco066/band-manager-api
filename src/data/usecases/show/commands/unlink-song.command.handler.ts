...
@CommandHandler(UnlinkSongCommand)
export class UnlinkSongHandler implements ICommandHandler<UnlinkSongCommand> {
  ...
  async execute(command: UnlinkSongCommand): Promise<Show> {
    ...
    return await this.removeSong(currentShow, currentSong)
  }
  ...
  // Fetch show from database
  async fetchSong(command: UnlinkSongCommand): Promise<Song | null> {
  ...
  validateRole(command: UnlinkSongCommand, band: Band, account: Account): void {
  ...
  // Remove song from show
  async removeSong(show: Show, song: Song): Promise<Show | null> {
    const { songs, id } = show
    const r = await this.showRepository.removeSong(
      songs.filter(s => s !== song._id.toString()), id
    )
    return r
  }
}
